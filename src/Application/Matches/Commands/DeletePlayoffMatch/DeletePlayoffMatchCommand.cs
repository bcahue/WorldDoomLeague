...
{
    public partial class DeletePlayoffMatchCommand : IRequest<bool>
    {
        public uint Match { get; set; }
    }

    public class UndoMatchCommandHandler : IRequestHandler<DeletePlayoffMatchCommand, bool>
    {
        ...
        public async Task<bool> Handle(DeletePlayoffMatchCommand request, CancellationToken cancellationToken)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                var roundFlagTouchCapture   = await _context.RoundFlagTouchCaptures     .Where(w => w.FkIdGame == request.Match).ToListAsync(cancellationToken);
                ...
                _context.RoundFlagTouchCaptures     .RemoveRange(roundFlagTouchCapture);
