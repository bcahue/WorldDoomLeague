...
{
    public class GetPlayerLeaderboardStatsBySeasonIdQuery : IRequest<PlayerLeaderboardSeasonStatsVm>
    {
        public uint Id { get; }
        public LeaderboardStatsMode Mode { get; }

        public GetPlayerLeaderboardStatsBySeasonIdQuery(uint id, LeaderboardStatsMode mode)
        {
            Id = id;
            Mode = mode; 
        }
    ...
    public class GetSeasonSummaryByIdQueryHandler : IRequestHandler<GetPlayerLeaderboardStatsBySeasonIdQuery, PlayerLeaderboardSeasonStatsVm>
    {
        private readonly IApplicationDbContext _context;

        public GetSeasonSummaryByIdQueryHandler(IApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<PlayerLeaderboardSeasonStatsVm> Handle(GetPlayerLeaderboardStatsBySeasonIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var seasonStats = await _context.StatsRounds
                    .Include(i => i.FkIdPlayerNavigation)
                    .Include(i => i.FkIdRoundNavigation)
                    .Where(w => w.FkIdSeason == request.Id)
                    .ToListAsync(cancellationToken);

            var playerStats = seasonStats.GroupBy(g => g.FkIdPlayer).Where(w => w.Count() >= 5); // Players who have more than 5
                                ...
                                case LeaderboardStatsMode.PerRound:
                                    {
                                        ...
                                        foreach (var capsStat in caps)
                                        {
                                            categoryTopStats.Add(new LeaderboardStatsDto
                                            {
                                                Id = i,
                                                PlayerName = capsStat.playerName,
                                                Stat = (double)capsStat.captureSum / (double)capsStat.roundsPlayed
                                            });
                                            i++;
                                        }
                                        break;
                                ...
                                case LeaderboardStatsMode.Per8Min:
                                    {
                                        ...
                                        foreach (var defensesStat in defenses)
                                        {
                                            categoryTopStats.Add(new LeaderboardStatsDto
                                            {
                                                Id = i,
                                                PlayerName = defensesStat.playerName,
                                                Stat = (defensesStat.defensesSum / defensesStat.timeMinutes) * 8.0
                                            });
                                            i++;
                                        }
                                        break;
                                ...
                                case LeaderboardStatsMode.Per8Min:
                                    {
                                        ...
                                        foreach (var fragsStat in frags)
                                        {
                                            categoryTopStats.Add(new LeaderboardStatsDto
                                            {
                                                Id = i,
                                                PlayerName = fragsStat.playerName,
                                                Stat = (fragsStat.fragsSum / fragsStat.timeMinutes) * 8
                                            });
                                            i++;
                                        }
                                        break;
            ...
            return new PlayerLeaderboardSeasonStatsVm
            {
                SeasonId = request.Id,
